...
async function populateEditGroupModal() {
    ...
    group_name         = spans[0].textContent;
    group_display_name = spans[1].textContent;
    group_description  = spans[2].textContent;
    group_id           = spans[3].textContent;
...
async function updateGroup() {
    ...
    $.ajax({
        ...
        url: "/update/group",
        data: {
            "original_name": h_e_group_name,
            "original_description": h_e_group_description,
            "original_group_id": h_e_group_id,
            "original_group_display_name": h_e_group_display_name,
            "new_name": e_group_name,
            "new_description": e_group_description,
            "new_group_id": e_group_id,
            "new_group_display_name": e_group_display_name
        },
        success: function (entry) {
...
window.addEventListener('DOMContentLoaded', async function main() {
    ...
    $('#table').DataTable({
        dom: 'Brtip', buttons: {
            buttons: [{text: 'Export', extend: 'csv', filename: 'User Mapping Report', className: 'btn-export'}], dom: {
                button: {
                    className: 'btn'
                }
            }
        ...
        initComplete: function () {
...

            {
                ...
                "width": "15%", "targets": 2, "render": function (data, type, row) {
                    let colorClass = "";
                    if (data === "yes") {
...
//console.log("window.addEventListener DOMContentLoaded END 1");
